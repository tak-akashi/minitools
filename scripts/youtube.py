...
async def main_async():
    ...
    parser.add_argument(
        "-u", "--youtube_url", "--url", required=True, help="YouTube動画のURL"
    )
    parser.add_argument(
        "-o", "--output_dir", default="outputs", help="出力ディレクトリ"
    )
    parser.add_argument(
        "-m",
        "--model_path",
    ...
    collector = YouTubeCollector(
        output_dir=str(output_dir), whisper_model=args.model_path
    )
    ...
    summary = await summarizer.summarize(
        result["transcript"], max_length=500, language="english"
    )
