    ...
    return [
        TrendWithTweets(
            ...
            tweets=[
                ...
                Tweet(
                    text="GPT-5 changes everything in AI",
                    retweet_count=3000,
                    like_count=10000,
                    author="user2",
                ),
        ...
        TrendWithTweets(
            ...
            tweets=[
                Tweet(
                    text="Claude 4 outperforms GPT-5",
                    retweet_count=2000,
                    like_count=8000,
                    author="user4",
                ),
...
class TestXTrendProcessor:
    ...
    async def test_process_full_pipeline(self, sample_trends_with_tweets):
        """フルパイプラインのテスト"""
        # フィルタリングレスポンス
        filter_response = json.dumps({"selected": [{"index": 0, "relevance": "LLM"}]})
        # 要約レスポンス
        summary_response = json.dumps(
            {
        ...
        class SequentialMockLLM(MockLLMClient):
            async def chat_json(self, messages, model=None):
                ...
                self.chat_calls.append(
                    {"messages": messages, "model": model, "json": True}
                )
                if call_count == 1:
    ...
    async def test_summarize_keyword_results(self):
        ...
        results = [
            KeywordSearchResult(
                ...
                tweets=[
                    Tweet(
                        text="Claude Code is great!", retweet_count=500, author="dev1"
                    ),
                    Tweet(
                        text="Using Claude Code for my project",
                        retweet_count=200,
                        author="dev2",
                    ),
                ],
...
class TestTimelineProcessing:
    ...
    async def test_filter_ai_tweets(self):
        """AI関連ツイートフィルタリングのテスト"""
        filter_response = json.dumps({"selected_indices": [0, 2]})
        mock_llm = MockLLMClient(json_response=filter_response)
        processor = XTrendProcessor(llm_client=mock_llm)
    ...
    async def test_summarize_timeline_results(self):
        ...
        results = [
            UserTimelineResult(
                ...
                tweets=[
                    Tweet(
                        text="New LLM reasoning research",
                        retweet_count=3000,
                        author="karpathy",
                    ),
                    Tweet(
                        text="Great weather today", retweet_count=10, author="karpathy"
                    ),
                ],
            ),
        ]
...
class TestProcessAll:
    ...
    async def test_process_all_basic(self):
        ...
        filter_tweets_response = json.dumps({"selected_indices": [0]})
